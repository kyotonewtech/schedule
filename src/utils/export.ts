...
export async function exportToExcel(weekStart: Date): Promise<void> {
  ...
  const worksheet = workbook.addWorksheet('役員スケジュール');
  ...
  titleCell.value = `役員スケジュール - ${formatWeekLabel(weekStart)}`;
  ...
  link.href = url;
  link.download = `役員スケジュール_${formatWeekLabel(weekStart).replace(/\s/g, '_')}.xlsx`;
  link.click();
