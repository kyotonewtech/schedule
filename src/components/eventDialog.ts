...
import { syncEventToGoogle, isAuthenticated } from '../services/google';
...
export class EventDialog {
  ...
  private async handleSubmit(): Promise<void> {
    // Google Calendar認証状態の事前チェック
    if (!isAuthenticated()) {
      const proceed = confirm(
        'Google Calendarに同期するには認証が必要です。\n' +
        '「Google連携」ボタンをクリックして認証してください。\n\n' +
        '認証せずにローカルのみに保存しますか？'
      );
      if (!proceed) {
        return;
    ...
    try {
      ...
      const syncResult = await syncEventToGoogle(savedEvent);
