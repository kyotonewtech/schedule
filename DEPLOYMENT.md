...
### 手順V-2: Google Cloud Console設定

**注意**: この手順はGoogle OAuth認証のための設定です。学内限定アクセスを実現するには：
- **Google Workspaceユーザーの場合**: OAuth同意画面を「内部」に設定（以下の手順）
- **Google Workspaceを持たない場合**: このセクションをスキップし、代わりに本ドキュメント末尾の「学内限定アクセス設定（IP制限）」を参照

#### OAuth同意画面を「内部」に設定（Google Workspaceユーザーのみ）

**前提条件**: Google Workspace（旧G Suite）を契約している組織のみ利用可能

1. [Google Cloud Console](https://console.cloud.google.com/)にアクセス
2. 「APIとサービス」→「OAuth同意画面」を開く
3. **User Type**を「内部」に変更
   - **内部**: Google Workspaceドメイン内のユーザーのみ（要Workspace契約）
   - **外部**: 誰でもログイン可能
4. 「保存」をクリック

**この設定により**:
- ✅ @your-domain.ac.jpドメインのユーザーのみログイン可能
- ✅ 外部ユーザーはログインボタンをクリックしてもエラーになる
- ✅ 追加コスト・複雑な実装なしで学内限定を実現

**Google Workspaceを持たない場合**:
- OAuth「内部」設定は利用不可
- 代わりに**Vercel IP Allowlist**を使用（本ドキュメント末尾を参照）
- 完全無料で学内ネットワークからのみアクセス可能
...
**重要**: Google Workspaceを持たない場合、Vercel無料プランでのIP制限は利用できません。

以下の3つの代替案から選択してください。

---

### 方法1: Vercel カスタムミドルウェア（パスワード認証）

**推奨度**: ⭐⭐⭐⭐ （無料で確実）

#### 概要
- Vercel Edge Middlewareで独自のパスワード認証を実装
- ログインページを作成し、正しいパスワードを入力したユーザーのみアクセス可能
- JWT tokenをHttpOnly cookieに保存

#### メリット
- ✅ **完全無料**（Vercel無料プランで利用可能）
- ✅ 学内・学外どちらからでもアクセス可能
- ✅ パスワード変更が容易
- ✅ 将来的にメールアドレス認証に拡張可能
...
#### デメリット
- ❌ 実装に2-3時間必要
- ❌ TypeScript/JavaScriptコードの知識が必要
- ❌ 全員が同じパスワードを共有（個別認証ではない）

#### コスト
- **$0** （完全無料）

#### 実装時間
- **2-3時間**（ログインページ作成 + Middleware実装）

#### 実装概要（詳細は別途提供可能）

1. `middleware.ts` を作成してパスワード認証ロジックを実装
2. `pages/login.html` でログインページを作成
3. 環境変数で安全にパスワードを管理
4. JWT tokenでセッション管理
...
#### 概要
- Vercelではなく学内のApache/Nginxサーバーでホスティング
- サーバー側でIP制限を設定
- 学内ネットワークからのみアクセス可能
...
- ✅ **完全無料**（既存インフラ利用）
- ✅ **最も確実なIP制限**
- ✅ 学内Active Directory/LDAP連携も可能
- ✅ 完全なデータ管理

#### デメリット
- ❌ 情報システム部門にサーバー割り当てを依頼する必要がある
- ❌ サーバー管理が必要（アップデート、バックアップ等）
- ❌ HTTPS証明書の設定が必要
...
#### 概要
- VercelからCloudflare Pagesに移行
- Cloudflare Accessでメールドメイン制限（@wakayama-med.ac.jp）
- 企業レベルのアクセス制御

#### メリット
- ✅ メールドメインで自動制限（@wakayama-med.ac.jp のみ）
- ✅ 個別ユーザー管理
- ✅ MFA（多要素認証）対応
- ✅ 詳細なアクセスログ
...
- ❌ 有料（月額$7〜20/ユーザー）
- ❌ Vercelからの移行作業が必要
- ❌ Cloudflareアカウント作成が必要

#### コスト
- **月額 $7-20** （5-10ユーザー想定）

#### 実装時間
- **30-60分**（Pages移行 + Access設定）
...
4. Cloudflare Access設定
   - Email domain rule: `*@wakayama-med.ac.jp`
5. デプロイ確認
...
| 項目 | カスタムミドルウェア | 学内サーバー | Cloudflare Pages |
|------|---------------------|-------------|------------------|
| **コスト** | $0 | $0 | $7-20/月 |
| **実装時間** | 2-3時間 | 1-2時間 | 30-60分 |
| **IT部門連携** | 不要 | 必要（サーバー割り当て） | 不要 |
| **学外アクセス** | ✅ 可能 | ❌ 不可 | ✅ 可能 |
| **IP制限** | ❌ 不可 | ✅ 可能 | ✅ 可能 |
| **個別認証** | ❌ 不可 | △ 可能（要実装） | ✅ 可能 |
| **セキュリティ** | 中 | 高 | 非常に高 |
| **メンテナンス** | 容易 | 要サーバー管理 | 容易 |
...
#### 完全無料が必須の場合
→ **方法1: カスタムミドルウェア** または **方法2: 学内サーバー**

#### IT部門の協力が得られる場合
→ **方法2: 学内サーバー**（最も確実）

#### 月額費用を払える場合
→ **方法3: Cloudflare Pages**（最も簡単・高機能）

#### 学外からもアクセスしたい場合
→ **方法1: カスタムミドルウェア** または **方法3: Cloudflare Pages**

---

### 注意: Vercel Firewallについて

**Vercel Firewallは無料プラン（Hobby）では利用できません。**

- **利用可能プラン**: Pro（$20/月）以上
- **無料プランの制限**: Firewallメニューが表示されない、またはアクセス不可

本ドキュメントの初版では「Vercel Firewallは無料で利用可能」と誤って記載していましたが、訂正いたします
